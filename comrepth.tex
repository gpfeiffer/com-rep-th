...
$G_{x} \subseteq \left\lange B \right\rangle$
...
Let $X$ and $Y$ be $G$-sets, then $X \sqcup Y$ and $X \times Y$ are $G$-sets. Denote by $[X]$ the isomorphism type of a $G$-set $X$ (e.g. $[X] = [G/G_{x}]$ if $X \conj_{G} G/G_{x}$). Then the set of all isomorphism types of $G$-sets can be regarded as a ring with addintion. $$[X] + [Y] = [X \sqcup Y]$$ $$[X] \cdot [Y] = [ X \times Y]$$  
\nocite{*}
